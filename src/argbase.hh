...
class argdescbase_t {
  public:
    using argdesc_vt = std::vector<argdescbase_t*>;
...
class argdesc_t : public argdescbase_t<Targ> {
  ...
  public:
    argdesc_t(const char* aName, const Tval& aDefault,
              setfun_t aSetFun, // member function of Targ to set argument/option
              const char* aDesc) :
        argdescbase_t<Targ>(aName, aDesc),
        _default(aDefault), _setfun(aSetFun) {}
    virtual ~argdesc_t() {}

  public:
    virtual std::ostream& print_usage(std::ostream& os) const { return os; }
    ...
    virtual int parse_args(int& currArg, const int noArgs, const char* argv[], Targ& aArg) const { return true; }
    virtual int parse_args2(uint& currArg, const std::vector<char*>& argv, Targ&) const { return true; }
...
class argdesc_t<Targ, bool> : public argdescbase_t<Targ> {
  public:
    ...
    using super_t = argdescbase_t<Targ>;

  public:
    argdesc_t(const char* aName, const val_t& aDefault,
              setfun_t aSetFun, // member function of Targ to set argument/option
              const char* aDesc) :
        argdescbase_t<Targ>(aName, aDesc),
        _default(aDefault), _setfun(aSetFun) {}
  ...
  public:
    virtual std::ostream& print_usage(std::ostream& os) const {
        os << "  " << std::setw(15) << super_t::name() << "  " << super_t::desc() << std::endl
           << std::string(19, ' ') << "default: " << default_value() << std::endl;
        return os;
    }
    virtual int parse_args(int& currArg, [[maybe_unused]] const int noArgs, const char* argv[], Targ& aArg) const {
  ...
  public:
    ...
    virtual int parse_args2(uint& currArg, const std::vector<char*>& argv, Targ& aArg) const {
        if (0 == strcmp(super_t::_name, argv[currArg])) {
            ...
            char* lEnd = 0;
            val_t lVal = static_cast<float>(strtod(argv[currArg], &lEnd));
            if (argv[currArg] == lEnd) {
                std::cerr << super_t::desc() << " needs float value" << std::endl;
                return -1;
            }
...
class argdesc_t<Targ, constcharptr_t> : public argdescbase_t<Targ> {
  ...
  public:
    argdesc_t(const char* aName, val_t& aDefault,
              setfun_t aSetFun, // member function of Targ to set argument/option
              const char* aDesc) :
        argdescbase_t<Targ>(aName, aDesc),
        _default(aDefault), _setfun(aSetFun) {}
...
template <class Targ>
bool parse_args(const int aStart, // starting index
                const int aNoArgs, const char* argv[], std::vector<argdescbase_t<Targ>*>& aArgDesc, Targ& aArgStruct) {
    int lCurrArg = aStart;
    while (lCurrArg < aNoArgs) {
        ...
        for (uint i = 0; i < aArgDesc.size(); ++i) {
            ...
            if (0 > lReturnCode) {
                return false;
            } else if (0 == lReturnCode) {
                continue;
            } else if (0 < lReturnCode) {
                lArgCouldBeSuccessfullyHandled = true;
                break;
            }
        }
        if (!lArgCouldBeSuccessfullyHandled) { return false; }
    }
    return true;
}
...
template <class Targ>
bool parse_args_collect(const int aStart, // starting index
                        const int aNoArgs, const char* argv[], std::vector<argdescbase_t<Targ>*>& aArgDesc, Targ& aArgStruct,
    ...
    while (lCurrArg < aNoArgs) {
        ...
        if (!lArgCouldBeSuccessfullyHandled) { aUnparsed.push_back(std::string(argv[lCurrArg++])); }
    }
    return true;
}

template <class Targ>
bool parse_args2(const uint aStart, // starting index
                 const std::vector<char*> argv, std::vector<argdescbase_t<Targ>*>& aArgDesc, Targ& aArgStruct) {
...
template <class Targ>
void print_usage(std::ostream& os, const char* aProgrammName, std::vector<argdescbase_t<Targ>*>& aArgDesc) {
